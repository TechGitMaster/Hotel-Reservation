...
export class DeclineComponent implements OnInit {
  ...
  funcCallMails(){
    this.numbMailsArr_notForAll = new Array<mails>();
    this.subs = this.service.getAcceptedDecline(this.numbCountSkip, this.numbCountLimit, 'declined' as string).subscribe((result) => {
      ...
      if(result.response !== 'no-data'){
        ...
        this.st_LoadingNoData = 'Empty declined mail...';
        this.numb_allMails = 0;
        this.lengthMails = 0;
      }
    }, (error) => 
  ...
  favoriteClick(number: number, favo_condition: any, condition: boolean): void{
    ...
    if(!this.condition_Delete){
      ...
      this.subs = this.service.saveNotFavorite(this.numbMailsArr_notForAll[numbers_count].IDS, favo_condition, 'decline').subscribe((result) => {
        this.subs.unsubscribe();
        if(condition){
  ...
  deleteMails(){
    if(this.arr_Checked.length > 0 && !this.condition_Delete){
      ...
      this.subs = this.service.deleteMails(this.arr_Checked, 'decline').subscribe(async (result) => {
        this.subs.unsubscribe();
